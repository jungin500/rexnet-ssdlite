...
def main(args):
    ...
    model = ssdlite224_rexnet_v1_lightning(
        ...
        is_freeze_base_net=True,
        num_classes=num_classes
    ).to(device)

    if resume_path is not None:
        # model.load_state_dict(torch.load(resume_path))
        model.load_from_checkpoint(resume_path)
    ...
    dataset = VOCDetectionDataset(
        ...
        folds='trainval',
        num_classes=num_classes,
        class_map={0: 'Perspn', 1: 'Vehicle', 2: 'Corn'},
        transform=Transform(),
    ...
    train_dataset, val_dataset = torch.utils.data.random_split(dataset, [trainset_items, validset_items])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', '-d', required=True, help='Dataset root path')
    parser.add_argument('--backbone-weight', '-bw', default=None, help='Backbone weight RexNetV1 W=1.0 path')
    parser.add_argument('--batch-size', '-b', type=int, default=32, help='Batch size')
    parser.add_argument('--num-workers', '-w', type=int, default=0, help='Workers (Recommended to set equals core size)')
    parser.add_argument('--num-classes', '-c', type=int, default=1000, help='Class count')
    parser.add_argument('--persistent-workers', '-p', action='store_true', help='Persistent worker thread')
    parser.add_argument('--resume', '-r', default=None, help='Resume weight path (RexNetSSDDetector)')
    parser.add_argument('--half', action='store_true', help='(RECOMMENDED) Half precision (FP16)')
    parser.add_argument('--freeze-base-net', '-f', action='store_true', help='(RECOMMENDED) Freeze backbone network')
